...
export class ReceiverService {
  ...
  getReceiver(
    ...
    if (cached) {
      try {
        ...
        return Promise.resolve(
          new Observable<Receiver>((subscriber) => {
    ...
    return this.authService.getBearerToken().then((token) => {
      ...
      const url = `/receiver/${encodeURIComponent(
        receiverId
      )}?userId=${encodeURIComponent(userId)}`;
      const obs = this.http.get<Receiver>(url, { headers: headers });
      obs.subscribe({
        next: (receiver) => {
          sessionStorage.setItem(cacheKey, JSON.stringify(receiver));
        },
      });
      return obs;
    });
  ...
  getReceiverEvents(
    ...
    return this.authService.getBearerToken().then((token) => {
      ...
      const url = `/events/${encodeURIComponent(
        receiverId
      )}?userId=${encodeURIComponent(userId)}`;
  ...
  addEvent(eventRequest: EventRequest): Promise<Observable<void>> {
    return this.authService.getBearerToken().then((token) => {
      const headers: HttpHeaders = new HttpHeaders({
        Authorization: token,
      });
      ...
      return this.http.post<void>(`/event`, eventRequest, { headers: headers });
    });
  }
  ...
  deleteEvent(
    ...
    return this.authService.getBearerToken().then((token) => {
      ...
      const url = `/event/${encodeURIComponent(
        eventId
      )}?userId=${encodeURIComponent(userId)}&receiverId=${encodeURIComponent(
        receiverId
      )}`;
