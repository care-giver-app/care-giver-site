...
export class EventService {
  ...
  public eventConfigs$ = this.eventConfigsSubject
    .asObservable()
    .pipe(filter((configs) => configs.length > 0));
  ...
  getEventColor(type: string): { primary: string; secondary: string } {
    const eventType = this.eventConfigsSubject.value.find(
  ...
  private formatEventTime(date: Date, long: boolean): string {
    ...
    const dateOfWeekOptions: Intl.DateTimeFormatOptions = {
    ...
    return `${dayOfWeekString} ${dateString} ${date.toLocaleTimeString(
      [],
      timeOptions
    )}`;
  }
