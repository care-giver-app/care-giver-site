...
@Component({
  selector: 'lib-care-receiver-selection',
  imports: [
    CommonModule,
    MatMenuModule,
    MatButtonModule,
...
export class UserListComponent implements OnInit {
  ...
  fetchReceivers() {
    this.authService.getCurrentUserId().then((userId) => {
      this.userId = userId;
      this.userService
        .getUserRelationships(this.userId)
        .then((relationships: Relationships | undefined) => {
          if (relationships) {
            const receiverIds = relationships.relationships.map(
              (r) => r.receiverId
            );
            this.receiverService
              .getReceivers(this.userId, receiverIds)
              .then((receivers: Receiver[]) => {
                this.receivers = receivers;
                if (this.receiverService.currentReceiverId) {
                  const currentReceiver = this.receivers.find(
                    (r) =>
                      r.receiverId === this.receiverService.currentReceiverId
                  );
                  if (currentReceiver) {
  ...
  submitAddReceiver() {
    this.userService
      .addCareReceiver(
        ...
        this.newReceiver.lastName
      )
      .then((resp) => {
        if (resp) {
          this.alertService.show(
            'Care Receiver added successfully',
            AlertType.Success
          );
          this.selectReceiver(resp.receiverId);
        } else {
          this.alertService.show(
            'Error adding care receiver. Please try again later.',
            AlertType.Failure
          );
        }
